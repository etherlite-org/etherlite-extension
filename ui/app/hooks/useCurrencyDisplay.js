...
import { getCurrentCurrency, getConversionRate, getNativeCurrency, getCurrentCurrencySymbol, getCurrentNetwork } from '../selectors' //XXX
import {setCurrentCurrencySymbol} from '../../app/store/actions'
...
export function useCurrencyDisplay (inputValue, { displayValue, prefix, numberOfDecimals, denomination, currency, ...opts }) {
  ...
  const currentCurrencySymbol = useSelector(getCurrentCurrencySymbol) //XXX
  const currentNetworkId = useSelector(getCurrentNetwork) //XXX
  ...
  useEffect(() => {
    if(currentNetworkId == 111 && suffix == 'ETL') {
      dispatch(setCurrentCurrencySymbol('ETL'))
    } else if(currentNetworkId == 142 && suffix == 'tETL') {
      dispatch(setCurrentCurrencySymbol('tETL'))
    } else {
      dispatch(setCurrentCurrencySymbol(suffix))
    }
  }, [currentNetworkId])
  ...
  } else if(currentNetworkId == 142) { //XXXL
    suffix = 'tETL'
  } 
  else{
    suffix = suffix
