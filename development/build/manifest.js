...
function createManifestTasks({ browserPlatforms }) {
    ...
    return Promise.all(
      browserPlatforms.map(async (platform) => {
        const platformModifications = await readJson(
          `${__dirname}/../../app/manifest/${platform}.json`
        );
  ...
  const envDev = createTaskForModifyManifestForEnvironment((manifest) => {
    ...
    console.log(
      "manifest.permissionsmanifest.permissionsmanifest.permissionsmanifest.permissions",
      manifest.permissions
    );
  });

  console.log("envDevenvDevenvDevenvDevenvDevenvDevenvDevenvDevenvDev", envDev);
